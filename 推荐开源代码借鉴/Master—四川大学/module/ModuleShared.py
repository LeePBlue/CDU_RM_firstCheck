# ----------------------------------------------
#           SCURM_Vision_Module_Shared
#               Coding By Pikachu
#                  进程内存共享
#
#            LAST_UPDATE:JUN01/2021
# ----------------------------------------------
...
class Shared:
    ...
    def Trys(in_conf, in_type):
        try:
            ...
            if shared_debug_flag:
                Debugs("Shared", "严重的问题:Lock", 2)
                Debugs("Shared", "TRY/" + in_type, 2)
            return None
    ...
    def Open(in_conf, in_type, optputs=True):
        try:
            open_temp = False
            for open_loop in range(0,shared_openmaxtry):
                if in_conf[in_type]["LOCK"]:
                    open_temp = True
                    break
                time.sleep(0.0001)
            if not open_temp:
                Debugs("Shared", "解锁失败:" + in_type + "", 2)
                return False
        except KeyError or BaseException:
            if shared_debug_flag:
                Debugs("Shared", "严重的问题:Lock", 2)
                Debugs("Shared", "OPE/" + in_type, 2)
            return False
        try:
            in_temp = in_conf[in_type]
            in_temp["LOCK"] = False
    ...
    def Ttl(in_conf, in_type, in_name):
        ...
        try:
            ...
            if shared_debug_flag:
                Debugs("Shared", "严重的问题:Type", 2)
                Debugs("Shared", "TTL/" + in_type + "/" + in_name, 2)
            Shared.Open(in_conf, in_type)
            return False
        try:
            in_conf[in_type] = in_temp
    ...
    def Del(in_conf, in_type, in_name):
        ...
        try:
            ...
            if shared_debug_flag:
                Debugs("Shared", "严重的问题:Type", 2)
                Debugs("Shared", "DEL/" + in_type + "/" + in_name, 2)
            Shared.Open(in_conf, in_type)
            return False
        try:
            if Shared.Ptr(in_conf, in_type, in_name) is None:
    ...
    def Get(in_conf, in_type, in_name, in_flag=True):
        ...
        try:
            ...
            if shared_debug_flag:
                Debugs("Shared", "严重的问题:Unkw", 2)
                Debugs("Shared", "GET/" + in_type + "/" + in_name, 2)
            Shared.Open(in_conf, in_type)
            return None
        try:
            in_conf[in_type] = in_temp
    ...
    def Put(in_conf, in_type, in_name, in_data, in_flag=True):
        ...
        try:
            ...
            if shared_debug_flag:
                Debugs("Shared", "严重的问题:Unkw", 2)
                Debugs("Shared", "PUT/" + in_type + "/" + in_name, 2)
            Shared.Open(in_conf, in_type)
            return None
        try:
            in_conf[in_type] = in_temp
